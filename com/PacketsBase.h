...
public:
	Encoding();
	bool BigEndianUnicode(const std::string &src, std::string &out);
	bool ASCII(const std::string &src, std::string &out);
	bool Unicode(const std::string &src, std::string &out);

private:
	bool Change(iconv_t cd, const std::string &src, std::string &out);
...
public:
	...
	CityInfo()
	{}
	CityInfo(std::string city, std::string building, int description, int x, int y, int z)
		:m_Location(x, y, z)
	{
		City = city;
		Building = building;
	...
	CityInfo(std::string city, std::string building, int x, int y, int z)
...
public:
	...
	{
		L_COND_V(!value.empty());

		string out;
		Encoding::Ins().ASCII(value, out);
		m_Stream.append(out);
		m_Stream.append(1, (char)0);
#if 0
		if (value == null)
		{
	...
	void WriteLittleUniNull(const std::string &value)
	{
		...
		string out;
		Encoding::Ins().Unicode(value, out);
		m_Stream.append(out);
		m_Stream.append(2, (char)0);
	...
	//@size 长度. 表示至少长度。 如果value.lengh()< size, 0填充满多出得。 如果 value.lengh() >= size, 增加m_Stream长度到value.lengh()
	void WriteLittleUniFixed(const std::string &value, int size)
	{
		...
		if ((int)out.length() >= size)
		{ //参考C#，留意没有空字符结尾得
			L_WARN("no empty char end?");
		}
		else
		{
			m_Stream.append(size - (int)out.length(), (char)0);
		}
	...
	void WriteBigUniNull(const std::string &value)
	{
		...
		string out;
		Encoding::Ins().BigEndianUnicode(value, out);
		m_Stream.append(out);
		m_Stream.append(2, (char)0);
